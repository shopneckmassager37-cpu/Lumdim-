...
import { Subject, Grade, Question, PracticeConfig, MaterialType, UserSettings } from '../types.ts';
import { generateQuestions, gradeOpenQuestion } from '../services/geminiService.ts';
...
import { 
  ...
  Trophy, ArrowRight, Loader2, Award, Info, Sparkles, UserPlus 
} from 'lucide-react';
    ...
    if (mode === 'TEST') {
      ...
      const gradingPromises = questions.map(async (q, idx) => {
        if (q.type === 'OPEN' && userAnswers[idx]) {
          const result = await gradeOpenQuestion(q.text, q.modelAnswer || "", userAnswers[idx]);
          return { idx, result };
        }
        return null;
      });
      ...
      results.forEach(res => {
        if (res) {
          ...
          onQuestionAnswered(questions[res.idx], res.result.score >= 50);
        }
      });
            ...
            <div className="p-8 md:p-12 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-gray-50 p-6 rounded-3xl text-center border border-gray-100">
                        <div className={`text-3xl font-black mb-1 ${score >= 80 ? 'text-green-600' : score >= 50 ? 'text-yellow-600' : 'text-red-500'}`}>{score}%</div>
                        <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">ציון סופי משוקלל</div>
                    </div>
                    <div className="bg-gray-50 p-6 rounded-3xl text-center border border-gray-100">
                        <div className="text-3xl font-black text-gray-800 mb-1">{questions.length}</div>
                        <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">שאלות בתרגול</div>
                    </div>
                    <div className="bg-gray-50 p-6 rounded-3xl text-center border border-gray-100">
                ...
                <div className="space-y-4 pt-4">
                    {isTeacher && (
                       <button 
                          ...
                          className="w-full bg-primary text-white py-5 rounded-2xl font-black text-xl shadow-lg hover:bg-blue-600 transition-all flex items-center justify-center gap-3"
                       >
                          <UserPlus size={24} />
                          <span>שייך לכיתה עכשיו</span>
                       </button>
                    )}
                    <button 
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-2xl mx-auto border border-gray-100">
          <div className="text-center mb-8">
             ...
             <h3 className="text-3xl font-bold text-gray-800 mb-2">{isTeacher ? 'הכנת תוכן כיתתי' : 'הגדרת תרגול'}</h3>
             <p className="text-gray-500">{isTeacher ? 'הזן נושא לייצור שאלות ותוכן עבור התלמידים' : 'התאם את חווית הלמידה שלך'}</p>
          </div>
          <div className="space-y-6">
             <div>
                ...
                <div className="flex-1">
                  ...
                  <div className="text-gray-700 leading-relaxed space-y-4">
                    ...
                    <div className="text-sm opacity-80 pt-2 border-t border-current/10">
                        <span className="font-bold">הסבר נוסף: </span>
