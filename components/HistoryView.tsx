...
import { CheckCircle, XCircle, Clock, ArrowRight, BrainCircuit, Sparkles, Target, TrendingUp, TrendingDown, Loader2, FileText, ChevronLeft, Award } from 'lucide-react';
...
const HistoryView: React.FC<HistoryViewProps> = ({ history, onBack, onOpenSummary, isTeacher }) => {
  ...
  const handleAnalyze = async () => {
    if (history.length < 3) {
      alert("אנא צבור לפחות 3 פריטים בהיסטוריה כדי לקבל ניתוח.");
      return;
    }
    ...
    try {
        ...
        alert("אירעה שגיאה בניתוח הנתונים. נסה שוב מאוחר יותר.");
    } finally {
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      {item.type === 'SUMMARY' ? (
                        <button 
                          ...
                          className="flex items-center gap-1 text-primary hover:text-blue-700 font-black text-xs transition-colors mx-auto"
                        >
